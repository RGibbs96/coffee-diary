...
function DisplayRecommendationCard(props) {

    const [recommendedBean, setRecommendedBean] = useState([])
    const [recommenderBean, setRecommenderBean] = useState([])
    const [originArray, setOriginArray] = useState([null, null, null])

    const allCoffees = props.brewedCoffees
    const allBeans = props.beans
    var randomRecommendationId
    var randomCoffee = []

    const fetchRecommenderBean = async () => {
        if (allCoffees.length !== 0) {
            let highlyRatedCoffee = allCoffees.filter(coffee => coffee.rating > 7.5)
            randomCoffee = highlyRatedCoffee[Math.floor(Math.random()*highlyRatedCoffee.length)]
            setRecommenderBean(randomCoffee)
        }
    }

    useEffect(() => {
        fetchRecommenderBean()
    }, [allBeans])

    const fetchRecommendations = async () => {
        let bean_id = recommenderBean.bean.id
        const url = `http://localhost:8000/api/vectors/${bean_id}/`
        const response = await fetch(url)

        if (response.ok) {
            const data = await response.json()
            const matchedIds = data["matched_bean_ids"]
            randomRecommendationId = matchedIds[Math.floor(Math.random()*matchedIds.length)]
            for (let bean of allBeans){
                if (bean.id == randomRecommendationId){
                    setRecommendedBean(bean)
                    let origins = [bean.origin1.name, null, null]
                    if (bean.origin2 != null){
                        origins[1] = (bean.origin2.name)
                    }
                    if (bean.origin3 != null){
                        origins[2] = (bean.origin3.name)
                    }
                    setOriginArray(origins)
                    break
                }
            }
        }
    ...
    // Creating origin sentence and tasting note sentence
    let originSentence = ""
    let tastingNoteSentence = ""
    if (recommendedBean.length !== 0) {
        originSentence += recommendedBean.origin1.name
        if (recommendedBean.origin2 !== null){
            originSentence += ` | ${recommendedBean.origin2.name}`
        }
        if (recommendedBean.origin3 !== null){
            originSentence += ` | ${recommendedBean.origin3.name}`
        }
        tastingNoteSentence += recommendedBean.note1
        if (recommendedBean.note2 !== null){
            tastingNoteSentence += `, ${recommendedBean.note2}`
        }
        if (recommendedBean.note3 !== null){
            tastingNoteSentence += `, ${recommendedBean.note3}`
        }
    }
    ...
    return (
        <div>
            {
            (recommendedBean.length !== 0) ?
            <div className="card" id="card">
                <div className="recommender-card-head">
                   <svg version="1.1" width="24px" height="45px">
                        <path d="M6.85714 32.3C6.85714 33.15 7.54286 34 8.57143 34H15.4286C16.4571 34 17.1429 33.15 17.1429 32.3V30.6H6.85714V32.3ZM12 0C5.31429 0 0 5.27 0 11.9C0 15.98 2.05714 19.55 5.14286 21.59V25.5C5.14286 26.35 5.82857 27.2 6.85714 27.2H17.1429C18.1714 27.2 18.8571 26.35 18.8571 25.5V21.59C21.9429 19.38 24 15.81 24 11.9C24 5.27 18.6857 0 12 0Z" fill="#FFFBF5"/>
                    </svg>
                    <span style={{paddingLeft:"8px"}}>Tip</span>
                </div>
                <div className="recommender-card-body">
                    Because you enjoyed {recommenderBean.bean.name}, we recommend the coffee below.
                </div>
                <img id="recommender-image" src={recommendedBean.picture_url} />
                <div className="recommender-card-body" style={{fontSize:"20px"}}>
                    {recommendedBean.name}
                </div>
                <div className="recommender-card-text">
                    <a id="roaster-url" href={recommendedBean.roaster.url}>{recommendedBean.roaster.name}</a>
                    <p>Origins: {originSentence}</p>
                    <p>Roast: {recommendedBean.roast_level}</p>
                    <p>Tasting Notes: {tastingNoteSentence}</p>
                </div>
            ...
            <div className="card" id="card">
                ...
                <div style={{textAlign:"center"}}>
                    <svg width="60%" height="50%" viewBox="0 0 623.65 623.63" preserveAspectRatio="xMidYMin" style={{paddingTop:"28px"}}>
                        ...
                        <defs>
                        <linearGradient id="paint0_linear_182_217" x1="0" y1="0" x2="626.295" y2="2.66805" gradientUnits="userSpaceOnUse">
...
export default DisplayRecommendationCard
